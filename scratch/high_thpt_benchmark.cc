...
wifi.SetRemoteStationManager ("ns3::ConstantRateWifiManager",
 ...
 // wifi.SetRemoteStationManager ("ns3::ConstantRateWifiManager","DataMode", StringValue ("OfdmRate54Mbps"),"ControlMode",StringValue ("OfdmRate54Mbps"));
  ...
  mac.SetType ("ns3::StaWifiMac",
               ...
               //"ActiveProbing", BooleanValue (false),"QosSupported", BooleanValue (false));
 ...
 Ptr<WifiNetDevice> dev = DynamicCast<WifiNetDevice> (apDevices.Get (0));
        ...
        std::cout<<"Contention window size: " << dca->GetMinCw()<<std::endl;
        	//dca->SetMinCw(32);
        	std::cout<<"Contention window size: " << dca->GetMaxCw()<<std::endl;
...
//
  ...
  /*uint16_t onoffPort = 4000;
  	...
  	onoffsinkApps.Stop (Seconds (20.0));
  	for (unsigned int i = 0; i < nWifi;i++)
  	{
   ...
   if (tracing)
    {
     ...
     // csma.EnableAsciiAll (ascii.CreateFileStream ("analysis/traffic_model/only_dnld_512.tr"));
       //csma.EnablePcapAll ("analysis/traffic_model/only_dnld_512", false);
  ...
  //phy.EnablePcap ("dnld", apDevices.Get (0));
  //csma.EnablePcap ("third", csmaDevices.Get (1), true);
  NS_LOG_INFO ("Run Simulation.");
  phy.SetPcapDataLinkType(YansWifiPhyHelper::DLT_IEEE802_11_RADIO);

    	//phy.EnablePcap ("analysis/traffic_model/sol/download", apDevices.Get (0));
    ...
    for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin (); i != stats.end () && count<30; ++i)
      {
        ...
        count++;
        tpt+=i->second.rxBytes * 8.0 / 50.0 / 1024 / 1024;
      }
