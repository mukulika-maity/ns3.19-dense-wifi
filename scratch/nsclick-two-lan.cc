...
int main (int argc, char *argv[])
{
  ...
  NetDeviceContainer d01 = csma.Install (NodeContainer (csmaNodes.Get (0), csmaNodes.Get (1)));
  NetDeviceContainer d12 = csma.Install (NodeContainer (csmaNodes.Get (1), csmaNodes.Get (2)));
  //NetDeviceContainer csmaDevices = csma.Install (csmaNodes);

  // Install normal internet stack on node B
  InternetStackHelper internet;
  internet.Install (csmaNodes.Get (1));
  internet.Install (csmaNodes.Get (2));

  // Install Click on node A
  ClickInternetStackHelper clickinternet;
  clickinternet.SetClickFile (csmaNodes.Get (0), "scratch/nsclick-lan-single-interface.click");
  ...
  ApplicationContainer recvapp = packetSinkHelper.Install (csmaNodes.Get (2));
  ...
  AddressValue remoteAddress (InetSocketAddress (Ipv4Address ("10.1.3.2"), 50000));
  ...
  // For tracing
  csma.EnablePcap ("nsclick-two-lan", d01, false);
