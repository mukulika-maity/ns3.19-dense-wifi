  ...
  uint16_t servPort = 50000;
   PacketSinkHelper onoffSink ("ns3::TcpSocketFactory",
    ...
    ApplicationContainer onoffsinkApps = onoffSink.Install (csmaNodes.Get (nCsma-1));
        ...
        onoffsinkApps.Stop (Seconds (150.0));
  for (unsigned int i = 0; i < nWifi;i++)
    {
      ...
      clientHelper.SetAttribute ("DataRate", StringValue ("20Kbps"));
   ...
   if (tracing)
    {
      ...
      csma.EnableAsciiAll (ascii.CreateFileStream ("tcp-bulk-send-aarf.tr"));
       csma.EnablePcapAll ("tcp-bulk-send-aarf", false);
    }
