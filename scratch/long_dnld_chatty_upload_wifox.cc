...
main (int argc, char *argv[])
{
	...
	QosWifiMacHelper qmac = QosWifiMacHelper::Default ();
	qmac.SetType ("ns3::ApWifiMac",
		"Ssid", SsidValue (ssid));

	NetDeviceContainer apDevices;
	apDevices = wifi.Install (phy, qmac, wifiApNode);
	...
	regmac->GetAttribute ("BE_EdcaTxopN",ptr);
	Ptr<EdcaTxopN> dca = ptr.Get<EdcaTxopN> ();
	NS_ASSERT (dca != NULL);
	...
	WebServerHelper webServer("ns3::TcpSocketFactory",
	InetSocketAddress (Ipv4Address::GetAny (), servPort));
	webServer.SetAttribute ("MaxBytes", UintegerValue (maxB));
	for(unsigned int j=0;j<80;j++){
	ApplicationContainer webServerApps = webServer.Install (csmaNodes.Get (nCsma-1));
	webServerApps.Start (Seconds (0.1));
	webServerApps.Stop (Seconds (500.0));
	...
	for (unsigned int i = 0; i < nWifi;i++)
	    {
	      ...
	      chattyClient.SetAttribute ("MaxTurns", UintegerValue (maxTurns));
	             ...
	             // if(j==0){
	              ...
	              //}
	              //else
	              chattyClientApps.Start (Seconds (j+1));
	...
	if (tracing)
	{
		...
		csma.EnablePcapAll ("analysis/traffic_model/11g_mod/final_results/ldsu_wifox_50q", false);
	...
	phy.EnablePcap ("analysis/traffic_model/11g_mod/final_results/ap_ldsu_wifox_50q", apDevices.Get (0));
	//AsciiTraceHelper ascii;
   // phy.EnableAscii(ascii.CreateFileStream ("analysis/traffic_model/11g_mod/ap_ldsu_wifox.tr"),apDevices.Get (0));
