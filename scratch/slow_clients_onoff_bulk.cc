...
main (int argc, char *argv[])
{
	...
	//WifiHelper slowwifi = WifiHelper::Default ();
	...
	for (unsigned int i = 0; i < nWifi; i++){
		...
		/*}
		else if((i==(nWifi-1)) || (i==(nWifi-2))){
			//slowwifi.SetStandard (WIFI_PHY_STANDARD_80211a);
			slowwifi.SetRemoteStationManager ("ns3::ConstantRateWifiManager",
						"DataMode", StringValue ("OfdmRate6Mbps"));
			staDevices.Add(slowwifi.Install (phy, mac, wifiStaNodes.Get(i)));
		}*/
	...
	staDevices.Add(slowwifi.Install (phy, mac, wifiStaNodes.Get(nWifi-2)));
	staDevices.Add(slowwifi.Install (phy, mac, wifiStaNodes.Get(nWifi-1)));
	...
	for (unsigned int i = 0; i < nWifi;i++)
	{
		...
		clientHelper.SetAttribute ("DataRate", StringValue ("25.5Kbps"));
	...
	if (tracing)
	{
		...
		csma.EnableAsciiAll (ascii.CreateFileStream ("analysis/traffic_model/logs/slow_clients_bulk_onoff.tr"));
		csma.EnablePcapAll ("analysis/traffic_model/logs/slow_clients_bulk_onoff", false);
  ...
  clientApps.Stop (Seconds (10.0));*/
	...
	phy.EnablePcap ("analysis/traffic_model/logs/slow_clients_bulk_onoff_ap", apDevices.Get (0));
