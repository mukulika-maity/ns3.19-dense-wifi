...
main (int argc, char *argv[])
{
	...
	webServerApps.Stop (Seconds (1000.0));
	...
	for (unsigned int i = 0; i < nWifi;i++)
	    {
	      ...
	      chattyClient.SetAttribute ("MaxTurns", UintegerValue (maxTurns));
	             ...
	             // if(j==0){
	              ...
	              chattyClientApps.Stop (Seconds (1000.0));
	    }
	}
	if (tracing)
	{
		...
		csma.EnablePcapAll ("analysis/traffic_model/11g_mod/final_results/ldsu_base_long_rtt", false);
	...
	phy.EnablePcap ("analysis/traffic_model/11g_mod/final_results/ap_ldsu_base_long_rtt", apDevices.Get (0));
