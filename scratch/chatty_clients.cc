...
main (int argc, char *argv[])
{
 ...
 /* Ptr<WifiNetDevice> dev = DynamicCast<WifiNetDevice> (apDevices.Get (0));
  ...
  uint16_t Chattyport = 1400;
  PacketSinkHelper sinkChatty ("ns3::TcpSocketFactory",
                           InetSocketAddress (Ipv4Address::GetAny (), Chattyport));
  ApplicationContainer sinkChattyApps = sinkChatty.Install (csmaNodes.Get (nCsma-1));
          sinkChattyApps.Start (Seconds (0.0));
          sinkChattyApps.Stop (Seconds (100.0));
  for (unsigned int i = 0; i < nWifi; i++){
                  std::cout << "i= " << i << std::endl;
                   BulkSendHelper sourceChatty ("ns3::TcpSocketFactory",
                                   InetSocketAddress (csmaInterfaces.GetAddress (nCsma-1), Chattyport));
                  // Set the amount of data to send in bytes.  Zero is unlimited.
                  sourceChatty.SetAttribute ("MaxBytes", UintegerValue ( chattyBytes));

                  for (unsigned int j = 0; j < 10; j++){
                	  ApplicationContainer sourceChattyApps = sourceChatty.Install (wifiStaNodes.Get (i));
                	  int seed = (i+1)*(j+1);
                 SeedManager::SetSeed (seed);
                 SeedManager::SetRun (1);
                  UniformVariable var;
                  int rand = var.GetInteger(0,65);
                  std::cout<<"var="<<rand<<std::endl;
                 sourceChattyApps.Start (Seconds (rand));
                  sourceChattyApps.Stop (Seconds (100.0));
                  }
   ...
   if (tracing)
    {
      ...
      csma.EnableAsciiAll (ascii.CreateFileStream ("tcp-chatty-send-aarf.tr"));
       csma.EnablePcapAll ("tcp-chatty-send-aarf", false);
    }
