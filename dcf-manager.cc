...
DcfState::UpdateBackoffSlotsNow (uint32_t nSlots, Time backoffUpdateBound)
{
  ...
  //std::cout<<"update backoff="<<m_backoffSlots<<"at time="<<Simulator::Now().GetSeconds()<<std::endl;
...
DcfState::StartBackoffNow (uint32_t nSlots)
{
  ...
  //std::cout<<"start backoff="<<nSlots<<"at time="<<Simulator::Now().GetSeconds()<<std::endl;
  m_backoffSlots = nSlots;
...
DcfManager::RequestAccess (DcfState *state)
{
   ...
   */
 // std::cout<<"sent packet at "<<this<<" at time= "<<Simulator::Now().GetSeconds()<<std::endl;
  if (state->GetBackoffSlots () == 0
      && IsBusy ())
    {
      ...
      //std::cout<<"collision at "<<this<<" at time= "<<Simulator::Now().GetSeconds()<<std::endl;
